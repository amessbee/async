...
ws.onopen = () => {
  ...
  const { dispatch, getBootstrap } = makeCapTP('clientCapTP', obj => ws.send(JSON.stringify(obj)));
  // Listen for WebSocket messages and dispatch to CapTP
  ws.on('message', data => { dispatch( JSON.parse(data) ); });

  // Get remote object and call its method
  const bootStrap = getBootstrap();
  let usersList = [];

  E(bootStrap).makeValidatedUsers()
  .then(_usersList => {
